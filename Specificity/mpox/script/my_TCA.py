...
def create_matrix(genome, list_pos, frame):
    ...
    for p in list_pos:
        ...
        mutation_motif = genome[p-2:p+1]
        if mutation_motif not in ['TCA', 'TGA']:
            print(f"Position {p} is not {mutation_motif} nucleotide in genome! It will not be taken into account when constructing the matrix.")
            continue
        elif mutation_pos == 'G':
...
flist = ["S1A.M11_A3A_Gordenin_TCA.txt","S1B.M11_A3B_Gordenin_TCA.txt"]
