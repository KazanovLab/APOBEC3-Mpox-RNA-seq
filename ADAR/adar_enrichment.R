...
for(s in 1:length(sample_list)){
  ...
  bcftools_grp[,muttype6 := muttype]
  bcftools_grp[muttype == "A>C", muttype6 := "T>G"]
  bcftools_grp[muttype == "A>G", muttype6 := "T>C"]
  bcftools_grp[muttype == "A>T", muttype6 := "T>A"]
  bcftools_grp[muttype == "G>A", muttype6 := "C>T"]
  bcftools_grp[muttype == "G>C", muttype6 := "C>G"]
  bcftools_grp[muttype == "G>T", muttype6 := "C>A"]
  
  dt <- bcftools_grp[,.(cnt=sum(bcftools_cnt)),by=muttype6]
  
  dt[, cnt := round(cnt)]
  dt[, total := sum(cnt)]
  dt[, "sample" := sample]

  dtapply <- dt[muttype6 %in% c("T>C")]
  
  dttrain <- dt[!muttype6 %in% c("T>C","C>T")]
  
  alldata <- rbind(alldata,dttrain)
  apldata <- rbind(apldata,dtapply)
}


# Estimate alpha and beta of beta-distribution, with known p
...
#nll <- function(log_kappa) {
#  kappa <- exp(log_kappa)
#  alpha <- p0 * kappa
#  beta  <- (1 - p0) * kappa
#  
  # log P(K_i | N_i, alpha, beta)
#  ll <- lchoose(N, K) + lbeta(K + alpha, N - K + beta) - lbeta(alpha, beta)
#  return(-sum(ll))
#}

#opt <- optim(par = log(10), fn = nll, method = "BFGS", hessian = TRUE)

#kappa_hat <- exp(opt$par)
#alpha_hat <- p0 * kappa_hat
#beta_hat  <- (1 - p0) * kappa_hat
...
apldata[, pval := (1- pbetabinom.ab(q = cnt - 1, size = total, shape1 = alpha_hat, shape2 = beta_hat))]

write.table(apldata,"/Users/mar/BIO/PROJECTS/MPOX/ADAR/results/adar_enrichment.txt",sep='\t',quote = F,row.names = F)
